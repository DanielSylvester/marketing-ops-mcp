...
const BrandSchema = z.string().refine(
  ...
  (val) => ({
    message: `Unknown brand: '${val}'. Configured brands: ${listConfiguredMetaBrands().join(", ") || "none"}`,
  })
);
...
export const meta_list_campaigns = {
  ...
  description:
    "List campaigns for a Meta ad account. Returns id, name, objective, effective_status. Filter by status if needed.",
  inputSchema: z.object({
    brand: BrandSchema,
...
export const meta_insights = {
  name: "meta_insights",
  description:
    "Get performance insights for a Meta account, campaign, or ad. Returns spend, impressions, clicks, ctr, leads. Date range required.",
  inputSchema: z.object({
    brand: BrandSchema,
    ...
    time_increment: z
      .union([z.literal("all_days"), z.string()])
      .optional()
      .default("all_days")
...
export const meta_get_creative = {
  ...
  description:
    "Get full creative spec for a Meta creative — image hashes, copy variants (titles/bodies/descriptions), CTAs, lead form ID, page IDs.",
  inputSchema: z.object({
    brand: BrandSchema,
...
export const meta_download_creatives = {
  ...
  description:
    "Download every image creative for a Meta campaign to a local folder. Resolves image hashes from the asset_feed_spec, fetches permalink URLs, writes JPGs. Returns a list of downloaded files.",
  inputSchema: z.object({
    brand: BrandSchema,
  ...
  async handler({ brand, campaign_id, dest_path }: { brand: Brand; campaign_id: string; dest_path: string }) {
    ...
    for (const ad of ads) {
      ...
      const imgResp = await c.get<{
        data: Array<{ hash: string; url: string; name?: string; width: number; height: number }>;
      }>(`/${c.accountId}/adimages`, { fields: "hash,url,name,width,height", hashes: JSON.stringify(hashes) });
...
export const meta_list_adsets = {
  ...
  description:
    "List ad sets for a Meta campaign or account. Returns id, name, campaign_id, daily_budget, status, targeting summary.",
  inputSchema: z.object({
    brand: BrandSchema,
  ...
  async handler({
    ...
    const data = await c.getPaginated<{
      id: string;
      name: string;
      campaign_id: string;
      daily_budget: string;
      ...
      targeting?: any;
    }>(`/${target}/adsets`, { fields: "id,name,campaign_id,daily_budget,effective_status,targeting" });
...
export const meta_list_ads = {
  ...
  async handler({
    ...
    const data = await c.getPaginated<{
      ...
      creative?: { id: string };
      effective_status: string;
    }>(`/${target}/ads`, { fields: "id,name,adset_id,creative{id},effective_status" });
...
export const meta_get_ad_account = {
  ...
  description:
    "Get detailed info for a Meta ad account — name, account_status, currency, timezone, balance, spend_cap, business_name.",
  inputSchema: z.object({
    brand: BrandSchema,
  }),
...
export const meta_create_campaign = {
  ...
  description:
    "[MUTATION] Create a new Meta campaign. Returns the new campaign ID. Always set status to PAUSED for review before activating.",
  inputSchema: z.object({
    brand: BrandSchema,
    name: z.string(),
    objective: z
      ...
      .describe("e.g. OUTCOME_LEADS, OUTCOME_SALES, OUTCOME_AWARENESS, OUTCOME_TRAFFIC"),
    ...
    special_ad_categories: z
      .array(z.string())
      .optional()
      .describe('e.g. ["HOUSING"], ["EMPLOYMENT"], ["CREDIT"] — required for regulated categories'),
    dry_run: z
      .boolean()
      .optional()
      .default(true)
      .describe("Default true. Pass false AND set MARKETING_OPS_MCP_EXECUTE=1 to actually create."),
  }),
  async handler(args: {
    brand: Brand;
    name: string;
...
export const meta_update_campaign = {
  ...
  description:
    "[MUTATION] Update an existing Meta campaign — pause, activate, rename, or change budget. Provide only the fields you want to change.",
  inputSchema: z.object({
    brand: BrandSchema,
    campaign_id: z.string(),
    ...
    dry_run: z
      ...
      .optional()
      .default(true)
      .describe("Default true. Pass false AND set MARKETING_OPS_MCP_EXECUTE=1 to actually update."),
  }),
  async handler(args: {
    brand: Brand;
    campaign_id: string;
...
export const meta_create_adset = {
  ...
  description:
    "[MUTATION] Create a new Meta ad set under a campaign. Returns the new ad set ID. Always set status to PAUSED for review.",
  inputSchema: z.object({
    brand: BrandSchema,
    campaign_id: z.string(),
    name: z.string(),
    ...
    optimization_goal: z
      ...
      .describe("e.g. LEAD_GENERATION, REACH, LINK_CLICKS, CONVERSATIONS"),
    targeting: z
      .record(z.any())
      .optional()
      .describe('JSON targeting object. e.g. {"geo_locations":{"countries":["IN"]},"age_min":25}'),
    status: z.enum(["ACTIVE", "PAUSED"]).default("PAUSED"),
    dry_run: z
      .boolean()
      .optional()
...
export const meta_update_adset = {
  ...
  description:
    "[MUTATION] Update an existing Meta ad set — pause, activate, rename, budget, targeting. Provide only fields to change.",
  inputSchema: z.object({
    brand: BrandSchema,
    adset_id: z.string(),
    name: z.string().optional(),
...
export const meta_create_ad = {
  name: "meta_create_ad",
  description:
    "[MUTATION] Create a new Meta ad under an ad set. Returns the new ad ID. Always set status to PAUSED for review.",
  inputSchema: z.object({
    brand: BrandSchema,
    adset_id: z.string(),
    name: z.string(),
    creative: z
      .record(z.any())
      .describe(
        'JSON creative object. Minimum: {object_story_spec:{page_id:"...",link_data:{message:"...",link:"...",image_hash:"..."}}}'
      ),
    status: z.enum(["ACTIVE", "PAUSED"]).default("PAUSED"),
    dry_run: z
      .boolean()
...
export const meta_update_ad = {
  name: "meta_update_ad",
  description:
    "[MUTATION] Update an existing Meta ad — pause, activate, rename, or replace creative. Provide only fields to change.",
  inputSchema: z.object({
    brand: BrandSchema,
    ad_id: z.string(),
    name: z.string().optional(),
